...
int main()
{
	...
	// Smart Pointer
	{
		...
		}
		{
			...
			shared_ptr<FStruct> Shared = make_shared<FStruct>();
			UniqueParam(Shared.get());
		}
		...
		//GSharedPtrStruct.reset(); // GSharedPtrStruct = nullptr;
		//PointerToSharedPointer.reset();
		if (!PointerToWeakPointer.expired())
		{
			std::cout << "Pointer는 살아있다!\n";
			GWeakPtrStruct.lock()->Hello();
		}
		else
		{
			std::cout << "Pointer는 소멸됨\n";
		}
	}
...
#pragma region auto
	{
		...
		// (전채 빌드 시간이 늘어남; 하지만 소소하게 쓰면 의미없는 시간이긴 하다)
...
#pragma region 람다(lambda)
	...
	}
	{
		...
		// 함수 내부에서 사용 가능하다
		// [a] 와 같이 사용하는 경우, a를 값으로 capture 한다

		// [&c]와 같이 &로 Capture하면 Reference로 가져온다
...
#pragma region random, sort
	{
		// 컴퓨터에서 random함수는 진짜 random이 아닐수 있습니다.
